...
import { getParameters } from 'apiSdk/parameters';
...
import { getImages } from 'apiSdk/images';
import { ImageInterface } from 'interfaces/image';

import { ImageListPage as ShowCaseList } from 'pages/images';

function DashboardPage() {
  const { session } = useSession();

  const parameterStat = useSWR<PaginatedInterface<ParameterInterface>>(
    () => `/parameters`,
    () => getParameters({ limit: 1, offset: 0 }),
  );

  const designStat = useSWR<PaginatedInterface<DesignInterface>>(
    () => `/designs`,
    () => getDesigns({ limit: 1, offset: 0 }),
  );

  const imageStat = useSWR<PaginatedInterface<ImageInterface>>(
    () => `/images`,
    () => getImages({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Parameters',
      result: parameterStat?.data?.totalCount,
      isLoading: parameterStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Designs',
      result: designStat?.data?.totalCount,
      isLoading: designStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Images',
      result: imageStat?.data?.totalCount,
      isLoading: imageStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`images`} passHref legacyBehavior>
                  <Button
