...
type ColumnType = ColumnDef<DesignInterface, unknown>;
...
export function DesignListPage(props: DesignListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getDesigns({
        ...
        order: params.order,
        searchTermKeys: ['design_name.contains', 'design_path.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DesignInterface>>(
    () => `/designs?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: DesignInterface) => {
    if (hasAccess('design', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/designs/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'design_name', header: 'Design Name', accessorKey: 'design_name' },
    { id: 'design_path', header: 'Design Path', accessorKey: 'design_path' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/designs/view/${record.id}`} passHref legacyBehavior>
                <Button
