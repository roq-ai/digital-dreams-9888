...
type ColumnType = ColumnDef<GalleryInterface, unknown>;
...
export function GalleryListPage(props: GalleryListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getGalleries({
        ...
        order: params.order,
        searchTermKeys: ['gallery_name.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<GalleryInterface>>(
    () => `/galleries?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: GalleryInterface) => {
    if (hasAccess('gallery', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/galleries/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'gallery_name', header: 'Gallery Name', accessorKey: 'gallery_name' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/galleries/view/${record.id}`} passHref legacyBehavior>
                <Button
