...
type ColumnType = ColumnDef<ImageInterface, unknown>;
...
export function ImageListPage(props: ImageListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getImages({
        ...
        order: params.order,
        searchTermKeys: ['image_type.contains', 'image_path.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ImageInterface>>(
    () => `/images?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ImageInterface) => {
    if (hasAccess('image', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/images/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'image_type', header: 'Image Type', accessorKey: 'image_type' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/images/view/${record.id}`} passHref legacyBehavior>
                <Button
